    ...
    return {
      ...
      link: function (scope, element, attrs) {
        element.on('click', function (event) {
          var printItem = event.currentTarget.attributes[4].value;
          printItem ? scope.printItem = printItem : false;
          scope.$apply();
          $timeout(function () {
            $window.print();
          });
...
AngularPrint.directive('printPortrait', function () {
  return {
    restrict: 'A',
    link: function () {
      var sheet = (function () {
      ...
      sheet.insertRule('@page{size:portrait;}', 0);
    },
  };
});
